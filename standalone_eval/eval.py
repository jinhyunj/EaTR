...
def compute_mr_r5(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10)):
    """If a predicted segment has IoU >= iou_thd with one of the 1st GT segment, we define it positive"""
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    pred_qid2window = {d["qid"]: [j[:2] for j in d["pred_relevant_windows"][:5]] for d in submission}  # :2 rm scores
    # gt_qid2window = {d["qid"]: d["relevant_windows"][0] for d in ground_truth}
    gt_qid2window = {}
    pred_qid2window_ = {}
    for d in ground_truth:
        ...
        cur_max_iou_idx = np.argmax(cur_ious)
        pred_qid2window_[cur_qid] = pred_qid2window[cur_qid][cur_max_iou_idx]
        gt_qid2window[cur_qid] = cur_gt_windows[0]

    qids = list(pred_qid2window.keys())
    pred_windows = np.array([pred_qid2window_[k] for k in qids]).astype(float)
    gt_windows = np.array([gt_qid2window[k] for k in qids]).astype(float)
    pred_gt_iou = compute_temporal_iou_batch_paired(pred_windows, gt_windows)
...
def eval_moment_retrieval(submission, ground_truth, save_filename, verbose=True):
    if "charades" in save_filename:
        length_ranges = [[0, 10], [10, 30], [0, 30],]  #
...
def eval_submission(submission, ground_truth, save_filename, verbose=True, match_number=True):
    """
    Args:
