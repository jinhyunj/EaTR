...
def eval_epoch_post_processing(submission, opt, gt_data, save_submission_filename):
    ...
    if opt.eval_split_name in ["val", "test"]:  # since test_public has no GT
        metrics = eval_submission(
            submission, gt_data, save_submission_filename,
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    if 'anet' in eval_loader.dataset.dset_name:
        post_processor = PostProcessor(
            clip_length=opt.clip_length, min_ts_val=0, max_ts_val=20000,
            min_w_l=0, max_w_l=20000, move_window_method="left",
...
def start_inference():
    ...
    # # Uncomment for checking computational complexity
    # start = time.time()
    # flops = FlopCountAnalysis(model, (torch.FloatTensor(1, 32, 512).cuda(), torch.ones((1, 32)).cuda(),torch.FloatTensor(1, 75, 2818).cuda(),torch.ones((1, 75)).cuda()))
    # print(flops.total())
